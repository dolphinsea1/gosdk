...
func JsonCompare(left, right map[string]interface{}, n int) (string, bool) {
	diff := &JsonDiff{HasDiff: false, Result: "", Differ: make([]JsonDiffer, 0)}

	jsonDiffDict(left, right, 1, diff, "")
	if diff.HasDiff {
		bytes, _ := json.Marshal(diff.Differ)
...
func jsonDiffDict(json1, json2 map[string]interface{}, depth int, diff *JsonDiff, keys string) {
		...
		if _, ok := json2[key]; ok {
			...
			case map[string]interface{}:
				if _, ok2 := json2[key].(map[string]interface{}); !ok2 {
					...
					diff.Differ = append(diff.Differ, JsonDiffer{
						Key:      quotedKey,
						NewValue: marshal(value),
						OldValue: marshal(json2[key]),
					})
				} else {
					diff.Result = diff.Result + "\n" + longBlank + quotedKey + ": "
					jsonDiffDict(value.(map[string]interface{}), json2[key].(map[string]interface{}), depth+1, diff, quotedKey)
				}
			case []interface{}:
				diff.Result = diff.Result + "\n" + longBlank + quotedKey + ": "
				if _, ok2 := json2[key].([]interface{}); !ok2 {
					...
					jsonDiffList(value.([]interface{}), json2[key].([]interface{}), depth+1, diff, quotedKey)
				}
			default:
...
func jsonDiffList(json1, json2 []interface{}, depth int, diff *JsonDiff, key string) {
	blank := strings.Repeat(" ", (2 * (depth - 1)))
	longBlank := strings.Repeat(" ", (2 * (depth)))
	diff.Result = diff.Result + "\n" + blank + "["
	size := len(json1)
	...
	for i := 0; i < size; i++ {
		...
		case map[string]interface{}:
			if _, ok := json2[i].(map[string]interface{}); ok {
				jsonDiffDict(json1[i].(map[string]interface{}), json2[i].(map[string]interface{}), depth+1, diff, key)
			} else {
				diff.HasDiff = true
				...
				diff.Differ = append(diff.Differ, JsonDiffer{
					Key:      key,
					NewValue: marshal(json1[i]),
					OldValue: marshal(json2[i]),
				})
			}
		case []interface{}:
			if _, ok2 := json2[i].([]interface{}); !ok2 {
				...
				jsonDiffList(json1[i].([]interface{}), json2[i].([]interface{}), depth+1, diff, key)
			}
		default:
